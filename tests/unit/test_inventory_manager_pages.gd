...
func test_grant_equipment_page_increments_and_emits() -> void:
	...
	var before := InventoryManager.get_inventory().unlocked_equipment_pages
	InventoryManager.grant_equipment_page()
	assert_eq(InventoryManager.get_inventory().unlocked_equipment_pages, before + 1)
	assert_signal_emitted(InventoryManager, "inventory_changed")

func test_award_fills_first_page_then_stops_at_capacity() -> void:
	PersistenceManager.save_game_data.inventory.unlocked_equipment_pages = 1
