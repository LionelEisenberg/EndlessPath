...
3. **Cycle** — open the cycling mini-game, follow the Madra Ball, click inflection points for XP
...
6. **Start an Adventure** — enter a procedurally generated hex map, move tile-by-tile spending stamina
...
| Item | Type | Source |
|------|------|--------|
| Dagger | Weapon (10 atk, MAIN_HAND) | NPC dialogue reward |
...
3. **Equipment doesn't affect combat** — the Dagger's `attack_power: 10` is cosmetic. `_get_attribute_bonuses()` returns 0
4. **Madra pools disconnected** — cycling Madra (ResourceManager) and combat Madra (VitalsManager) are separate systems with no link. Design for unification in [RESOURCES.md](infrastructure/RESOURCES.md)
